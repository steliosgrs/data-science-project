...
for i in datasets:
    clean_data(i, ignore_negatives=True, ignore_zeros=True, ignore_eu=True)
    rename_countries_and_cats(i)

# df_in_percentages = drop_negatives(df_in_percentages)  #uncomment to remove 0s and negatives from cleaned dataset
...
if __name__ == '__main__':
    ...
    # ???????????????????????????????????????????????????????????????????????????????????????????????????????????
    zeros = df_in_millions.loc[df_in_millions['Value'] == 0]
    print(len(zeros))
